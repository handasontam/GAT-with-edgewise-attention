...
def load_eth_data():
    ...
    # store sparse adj matrix for each edge attribute to disk
    adjs = []
    edge_attr_name = ['sum_value', 'avg_value', 'var_value', 
                        'count_trans', 'count_call', 'count_mining', 
                        # 'count_other'
                    ]
    for attr in edge_attr_name:
        adj = sp.load_npz('/home/handason/data/adj_{}.npz'.format(attr))
        variance = np.var(adj.data)
        print(attr)
        # print(variance)
        adj.data = adj.data/variance
        adjs.append(adj)
    features = pd.read_csv('/home/handason/data/node_features_with_label_filtered.csv', delimiter=',')
    labels = features.label
    features = features.drop(['label', 'address', 'id'], axis=1).values  # N * 23
    
    # convert label to one-hot format
    labels = pd.get_dummies(data=labels, dummy_na=True, columns=['label']) #  N * 8
    size = labels.shape[0]
    train_ratio = 0.8
    ...
    for column in labels.columns:
        if (type(column) == str):
            set_of_key = set(labels[(labels[column] == 1) & (labels[np.nan] == 0)].index)
            ...
            train_mask[list(train_key_set)] = 1
            val_mask[list(test_key_set)] = 1
            test_mask[list(test_key_set)] = 1
